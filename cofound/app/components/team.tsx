...
export default function Team7({
    ...
    switch (type) {
      case 'facebook':
        return (
          <Link
            href={url}
            className="text-gray-400 transition-colors hover:text-purple-500 transition-colors duration-300"
          >
            <Facebook size={18} />
          </Link>
        );
  ...
  return (
    <section
      className={cn('w-full py-20 text-white text-center  ', className)}
    ...
    >
      <div className="container mx-auto max-w-5xl px-4">
        <div className="mb-12">
          ...
          <h2
            className={cn(
              'mb-6 text-4xl  text-center leading-tight font-bold md:text-5xl',
              
            )}
          >
            {title}
          </h2>
          <p className="text-center text-[#9E9E9E]" style={{ color: secondaryColor }}>
            {description}
          </p>
        </div>
        ...
        <div className="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-4">
          {teamMembers.map((member) => (
            <div key={member.id} className="group">
              ...
              <div className="mt-2 flex justify-center  items-center space-x-4 text-center ">
  {member.socialMedia &&
